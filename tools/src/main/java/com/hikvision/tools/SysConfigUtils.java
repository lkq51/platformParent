 ...
 * 系统配置管理工具
 * ====================================================
 * Modified by zhanhongkui at 2015-12-24
 * Modified reason: 对系统配置工具名称进行调整，并剥离对Spring的依赖。
 * @author shenqiujun on 2015/12/22
 */
public class SysConfigUtils {
    ...
    /**是否为单机字段，0标识为单机，1为集群**/
    private static final String ISSIGNLENODE = "0";
     ...
     * 重新加载配置文件
     *
     * @Modified By zhanhongkui at 2016-01-04
    ...
    private static void loadHbase() {
        ...
        try {
            ...
            for (Element e : list) {
                if ("hbase.zookeeper.quorum".equalsIgnoreCase(e.element("name").getText().trim())) {
    ...
    /**
     * 修改属性文件
     * @author chengqiujiang 2015-12-05 11:17:00
     * @param key
     * @param value
     * @param file
     */
    ...
    public static String[] getStringArray(String key) {
        return getCompositeConfig().getStringArray(key);
    }

    public static List<String> getStringList(String key) {
        return Arrays.asList(getCompositeConfig().getStringArray(key));
    }
    ...
    /**
    * @Description: 根据是否是集群，修改上传的hdfs路径
    * @Author: fengyouxu
    * @Date: 2018/5/21
    */
    public static String getSparkYarnJARs() {
        if(ISSIGNLENODE.equals(getCompositeConfig().getString("isSingleNode"))){
            return getCompositeConfig().getString("spark.yarn.jars");
        }else{
            String[] hadoopFileList = getCompositeConfig().getString("hadoop.home").split("/");
            String sparkJarPath = "hdfs://"+hadoopFileList[hadoopFileList.length-1]+"/sparkjars/jars/*";
            LOGGER.debug("sparkJarPath********:"+sparkJarPath);
            return sparkJarPath;
        }
    }
    public static String getSparkYarnStagingDir() {
        if(ISSIGNLENODE.equals(getCompositeConfig().getString("isSingleNode"))){
            return getCompositeConfig().getString("spark.yarn.stagingDir");
        }else{
            String[] hadoopFileList = getCompositeConfig().getString("hadoop.home").split("/");
            String sparkJarPath = "hdfs://"+hadoopFileList[hadoopFileList.length-1]+"/sparkjars/jars/";
            LOGGER.debug("sparkJarPath********:"+sparkJarPath);
    ...
    protected static List<String> replaceIps2List(String ips) {
        ...
        for (String candidate : array) {
            if (CommonUtils.PATTERN_IP_ADDR.matcher(candidate).matches()) {
                list.add(candidate);
    ...
    public static void reloadPipelineFiles() {
        reload();
    }
